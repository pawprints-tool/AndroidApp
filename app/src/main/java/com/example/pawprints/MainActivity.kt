...
//    private var MEASUREMENT_PORT: Int = -1
    ...
    private fun updatePermissions()
    {
        ...
        mWiFiStatePermission = ContextCompat.checkSelfPermission(this@MainActivity, Manifest.permission.CHANGE_WIFI_STATE) == PackageManager.PERMISSION_GRANTED
    }
    ...
    private fun requestPermissions()
    {
        ...
        {
            permissionArray.add(Manifest.permission.CHANGE_WIFI_STATE)
        }
    ...
    private val mCaptureWiFiMeasurements: Runnable = object : Runnable{
            ...
            thread{
                kotlin.run {
                    try {
                        ...
                        val jsonLog = WifiMeasurementsHandler().getInfo(wifiManager, SettingsHandler, "json", mMeasurementCampaignName, mIMEI, mExpStartElapsedNanoSec )

                        if (SettingsHandler.LogLocally)
                        {
                            thread {
                                mLogHandler.appendToWiFiMeasurements(
...
//                        if (success) {
//                            var eventDescriptionJSON = JSONObject()
//                            eventDescriptionJSON.put("abs_time", System.currentTimeMillis())
//                            val nowElapsed = (SystemClock.elapsedRealtimeNanos() - mExpStartElapsedNanoSec)/Math.pow(10.0, 9.0)
//                            eventDescriptionJSON.put("rel_time", nowElapsed)
//                            eventDescriptionJSON.put("event_description", "wifi_scan_succeeded")
//
//                            mLogHandler.appendToManualEventLog(
//                                mStorageWritePermission,
//                                mMeasurementCampaignName,
//                                eventDescriptionJSON.toString()
//                            )
//                            val jsonLog = WifiMeasurementsHandler().getInfo(
//                                wifiManager,
//                                SettingsHandler,
//                                "json",
//                                mMeasurementCampaignName,
//                                mIMEI,
//                                mExpStartElapsedNanoSec
//                            )
//                            if (SettingsHandler.LogLocally) {
//                                thread {
//                                    mLogHandler.appendToWiFiMeasurements(
...
//                        }
                    ...
                    try
                      {
                         mHandler.postDelayed(this, SettingsHandler.LogInterval)
                          ...
                          var jsonLog = CellMeasurementsHandler().getInfo(telephonyManager, connectivityManager, SettingsHandler, "json", mMeasurementCampaignName, mIMEI, mExpStartElapsedNanoSec )
        ...
        override fun run()
        {
            ...
            kotlin.run {
                try {
                    ...
                    val textView: TextView = findViewById(R.id.logText)
                    var displayString = ""
                    val telephonyManager = getSystemService(TELEPHONY_SERVICE) as TelephonyManager
                    val connectivityManager = getSystemService(CONNECTIVITY_SERVICE) as ConnectivityManager
                    displayString += CellMeasurementsHandler().getInfo(telephonyManager, connectivityManager, SettingsHandler, "display",  mMeasurementCampaignName, mIMEI, mExpStartElapsedNanoSec)
    ...
    override fun onResume()
    {
        ...
        if(SettingsHandler.MarkEvents)
        {
            val markBtn: Button = findViewById(R.id.recordEvent)
            val eventDescrText: EditText = findViewById(R.id.eventDescription)
            markBtn.visibility = View.VISIBLE
            eventDescrText.visibility = View.VISIBLE
            markBtn.setOnClickListener {
                try {
                    val eventDescrText: EditText = findViewById(R.id.eventDescription)
                    val eventDescription = eventDescrText.text.toString()
                    var eventDescriptionJSON = JSONObject()
                    eventDescriptionJSON.put("abs_time", System.currentTimeMillis())
                    eventDescriptionJSON.put("rel_time", this.getElapsedTime())
                    eventDescriptionJSON.put("event_description", eventDescription)
                    mLogHandler.appendToManualEventLog(mStorageWritePermission, mMeasurementCampaignName, eventDescriptionJSON.toString())
                    Toast.makeText(this, "Recorded event.", Toast.LENGTH_SHORT).show()
                }
                catch (t: Throwable)
                {
                    mLogHandler.appendToDebug(mStorageWritePermission, mDebugLogFile, t.toString())
...
//            val logScroll: ScrollView = findViewById(R.id.logScroll)
//            val scrollLayout = logScroll.layoutParams as ViewGroup.MarginLayoutParams
//            scrollLayout.topMargin = 180
//            logScroll.layoutParams = scrollLayout
        }
        else
        {
            val markBtn: Button = findViewById(R.id.recordEvent)
            ...
            eventDescrText.visibility = View.GONE
//            val logScroll: ScrollView = findViewById(R.id.logScroll)
//            val scrollLayout = logScroll.layoutParams as ViewGroup.MarginLayoutParams
//            scrollLayout.topMargin = 130
//            logScroll.layoutParams = scrollLayout
        }
    ...
    private fun start(){
        try {
            ...
            thread{
                ...
                thread{
                    kotlin.run{
                        if(SettingsHandler.LogWiFi){
                            mHandler.postDelayed(mCaptureWiFiMeasurements, SettingsHandler.LogInterval)
                        }
                    }
                }
