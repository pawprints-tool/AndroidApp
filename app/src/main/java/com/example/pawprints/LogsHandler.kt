...
class LogsHandler(logFolder:String)
{
    ...
    public fun appendToWiFiMeasurements(storagePermission:Boolean, campaignName:String, toAppend:String)
    {
        if(!storagePermission)
        {
            return
        ...
        val folderName = File(path, mlogFolder)
        folderName.mkdirs()
        val file = File(folderName, campaignName + "_wifi_log.txt")
        file.appendText(toAppend + "\n")
    }

    public fun appendToManualEventLog(storagePermission:Boolean, campaignName:String, toAppend:String)
    {
        if(!storagePermission)
        {
            return
        ...
        val folderName = File(path, mlogFolder)
        folderName.mkdirs()
        val file = File(folderName, campaignName + "_event_log.txt")
        file.appendText(toAppend + "\n")
    }
